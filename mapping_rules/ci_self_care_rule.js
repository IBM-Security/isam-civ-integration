    ...
    if(userId != null) {
        ...
        if(action == null || action == "initiate") {
            ...
            if(type == "verify") {
                ...
                var resp = CiClient.postRequest(conn, "/v1.0/authenticators/initiation?qrcodeInResponse=true", JSON.stringify(registrationJson), getLocale());
                ...
                if(type == "verify") {
                    ...
                    for(j = 0; j < authenticators.length; j++) {
                        if(authenticators[j].id == id) {
                            json = authenticators[j];
                            break;
                        }
                    }
                    if(!json.id) {
                        // No valid ID was supplied. Return an error page via our
                        // handleError method (defined in CI_Common.js).
                    ...
                    var resp = CiClient.updateAuthenticator(conn, id, JSON.stringify(json), getLocale());
                    if (resp != null && resp.getCode() == 204) {
                        // Clean the state. cleanState is defined in CI_Common.js
                        // Check the function definition to confirm which state variables
                        // are cleared.
                        ...
                        if(authMethodJson.authnmethods.length > authMethods.length) {
                            for(i = 0; i < authMethodJson.authnmethods.length; i++) {
                                if(authMethodJson.authnmethods[i].methodType == "totp") {
                                    ...
                                    macros.put("@STATUS@", "successfulTOTP");
                                    break;
                                }
                            }
                        }
